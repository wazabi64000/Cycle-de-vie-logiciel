...
async function init() {
  ...
  const [indexRes, meta] = await Promise.all([
    fetch('/cours-cda/index.json').then((r) => r.json()),
    fetch('/cours-cda/index.json')
      .then((r) => r.json())
      .then((d) => d.modules.find((m) => m.slug === slug)),
  ]);
  ...
  courseFrame.src = `/cours-cda/${slug}/cours.html`;

  const fullCourseLink = document.getElementById('open-full-course');
  if (fullCourseLink) {
    fullCourseLink.href = `/cours-cda/${slug}/cours.html`;
