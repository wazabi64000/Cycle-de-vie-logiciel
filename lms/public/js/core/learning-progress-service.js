...
class LearningProgressService {
  ...
  getResumeUrl() {
    const pos = this.getLastPosition();
    if (!pos.moduleId) return 'dashboard.html';
    const params = new URLSearchParams({ slug: pos.moduleId, resume: '1' });
    if (pos.chapterId) params.set('chapter', pos.chapterId);
    if (pos.lessonId) params.set('lesson', pos.lessonId);
